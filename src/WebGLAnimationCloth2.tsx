...
// https://threejs.org/examples/#webgl_animation_cloth2
...
const xSegs = 20, ySegs = 15
const clothWidth = 9, clothHeight = 6
const startX = -4.5, startY = 13
const numX = xSegs + 1
const totalParticles = numX * (ySegs + 1)
...
const init3D = (container: HTMLElement) => {
    ...
    const pole = new THREE.Mesh(
        new THREE.CylinderGeometry(0.08, 0.08, 13, 8),
        new THREE.MeshPhongMaterial({ color: 0xaaaaaa })
    )
    pole.position.set(-4.5, 6.5, 0)
    ...
    const params = { windStrength: 0.8, gravity: 0.1, relaxIterations: 5 }
    ...
    const clothMat = new THREE.MeshPhongMaterial({ color: 0xff2244, side: THREE.DoubleSide })
    ...
    for (let iy = 0; iy <= ySegs; iy++) for (let ix = 0; ix <= xSegs; ix++) {
        ...
        const x = startX + (ix / xSegs) * clothWidth
    ...
    arrowGeo.setFromPoints([new THREE.Vector3(0, 2, 0), new THREE.Vector3(2, 2, 0)])
    const arrow = new THREE.Line(arrowGeo, new THREE.LineBasicMaterial({ color: 0xffff00, linewidth: 2 }))
    scene.add(arrow)
    ...
    cone.position.set(2.25, 2, 0)
    cone.rotation.set(0, 0, -Math.PI / 2)
    scene.add(cone)

    const panel = new GUI()
    panel.add(params, 'windStrength', 0, 2, 0.01).name('Wind Strength')
    panel.add(params, 'gravity', 0, 0.5, 0.005).name('Gravity')
    panel.add(params, 'relaxIterations', 1, 20, 1).name('Relax Iterations')
    ...
    const animate = () => {
        ...
        const t = time * 0.001
        const windX = Math.sin(t * 2) * params.windStrength
        const windY = Math.sin(t * 1.3) * params.windStrength * 0.05
        const gravY = -params.gravity

        for (let i = 0; i < totalParticles; i++) {
            if (pinned[i]) continue
            const px = positions[i * 3], py = positions[i * 3 + 1], pz = positions[i * 3 + 2]
            const ppx = prevPositions[i * 3], ppy = prevPositions[i * 3 + 1], ppz = prevPositions[i * 3 + 2]
            const nx = 2 * px - ppx + windX * 0.0016
            const ny = 2 * py - ppy + (gravY + windY) * 0.0016
            const nz = 2 * pz - ppz
            prevPositions[i * 3] = px; prevPositions[i * 3 + 1] = py; prevPositions[i * 3 + 2] = pz
            positions[i * 3] = nx; positions[i * 3 + 1] = ny; positions[i * 3 + 2] = nz
        }
        const iters = Math.round(params.relaxIterations)
        ...
        const scale = Math.abs(params.windStrength)
        arrow.scale.set(scale, 1, 1)
        cone.position.set(scale * 2 + 0.25, 2, 0)
...
export default function WebGLAnimationCloth2() {
