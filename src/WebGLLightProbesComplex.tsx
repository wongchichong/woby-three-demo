...
const init3D = (container: HTMLElement) => {
	...
	let probesLeft: InstanceType<typeof LightProbeGrid> | null = null;
	let probesRight: InstanceType<typeof LightProbeGrid> | null = null;
	let probesHelperLeft: InstanceType<typeof LightProbeGridHelper> | null = null;
	let probesHelperRight: InstanceType<typeof LightProbeGridHelper> | null = null;
	...
	function bakeWithResolution(resolution: number) {
	...
	gui.add(params, 'enabled').name('GI').onChange((value: boolean) => {
		if (probesLeft) probesLeft.visible = value;
		if (probesRight) probesRight.visible = value;
	});
	...
	gui.add(params, 'showProbes').name('Show Probes').onChange((value: boolean) => {
		if (probesHelperLeft) probesHelperLeft.visible = value;
		if (probesHelperRight) probesHelperRight.visible = value;
	});
	...
	_cleanupFn = () => {
		...
		if (gui) gui.destroy();
		if (probesLeft) probesLeft.dispose();
		if (probesRight) probesRight.dispose();
		if (probesHelperLeft) probesHelperLeft.dispose();
		if (probesHelperRight) probesHelperRight.dispose();
		controls.dispose();
		renderer.dispose();
		room.geometry.dispose();
		(room.material as THREE.Material).dispose();
		redWall.geometry.dispose();
		(redWall.material as THREE.Material).dispose();
		blueWall.geometry.dispose();
		(blueWall.material as THREE.Material).dispose();
		dividerLeft.geometry.dispose();
		(dividerLeft.material as THREE.Material).dispose();
		dividerRight.geometry.dispose();
		(dividerRight.material as THREE.Material).dispose();
		lintel.geometry.dispose();
		(lintel.material as THREE.Material).dispose();
		columnGeom.dispose();
		...
		(step1.material as THREE.Material).dispose();
		step2.geometry.dispose();
		(step2.material as THREE.Material).dispose();
		step3.geometry.dispose();
		(step3.material as THREE.Material).dispose();
		...
		(sculpture.material as THREE.Material).dispose();
...
export default function WebGLLightProbesComplex() {
	return (
		<div
			ref={(el) => {
				if (el) init3D(el);
